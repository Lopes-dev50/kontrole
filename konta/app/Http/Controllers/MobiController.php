...
class MobiController extends Controller
{
    ...
    ->get();
        ...
        ->get(['etiqueta']);
           ...
           return view('/mobi/painel', ['etiqueta' =>$etiquetas_distintas , 'totalmeta' => $totalMeta, 'filtros' => $filtros, 'datae' => $datae, 'data3' => $data3,  'data1' => $data1, 'labelse' => $labelse, 'labels1' => $labels1, 'moviments' => $moviments, 'dinheiros' => $dinheiros, 'totalEntradaPorAno' => $totalEntradaPorAno, 'totalSaidaPorAno' => $totalSaidaPorAno, 'totalEntradaPorMes' => $totalEntradaPorMes, 'totalSaidaPorMes' => $totalSaidaPorMes, 'dadosPizza' => $dadosPizza, 'labels' => $labels, 'data2' => $data2]);
            ...
            return view('mobi/renovar', [ 'valormes' => $valormes, 'valorano' => $valorano]);
        }
   ...
   return view('mobi.create');
    ...
    if ($dinheiro) {
        return view('mobi.show')->with('dinheiro', $dinheiro);
    } else {
        //..senão, retorna a view com uma mensagem que será exibida.
        return view('mobi.show')->with('msg', 'Veículo não encontrado!');
    }
    }
   ...
   //==============================================================
    public function edit($id)
    {
        ...
        return view('/mobi/editar', ['dinheiros' => $dinheiros]);
    }
    ...
    public function update($id, $mes, $ano,  Request $request) //atualiza um recurso informado mediante o parâmetro $id com os dados vindos do cliente, presentes no objeto Request;
    {
        ...
        // $dinheiro->data = $request->input('data');
         ...
         return redirect()->route('mbcada.mes', ['mes' => $mes, 'ano' => $ano])->with('success', 'Atualizado com sucesso!');
    }
    ...
    $this->Logger->log('info', 'Deletou um registro');
     ...
     if($total->isEmpty() ){
       ...
       return redirect()->route('mbcontrole.index')->with('success', 'Deletado com sucesso!');
     }else{
        $mes = $this->Enc->encriptar($mes);
        $ano = $this->Enc->encriptar($ano);
        return redirect()->route('mbcada.mes', ['mes' => $mes, 'ano' => $ano])->with('success', 'Deletado com sucesso!');
     }
...
->whereYear('data', '=', $ano)->whereMonth('data', '=', $mes)->where('user_id', '=', $user_id)->orderBy('id', 'desc')->get();
                ...
                return view('/mobi/cadames', ['dataAtual' => $dataAtual, 'filtros' => $filtros, 'dinheiros' => $dinheiros,'datae' => $datae, 'data3' => $data3,  'data1' => $data1, 'labels1' => $labels1, 'labelse' => $labelse, 'totalEntradaPorMes' => $totalEntradaPorMes, 'totalSaidaPorMes' => $totalSaidaPorMes, 'dadosPizza' => $dadosPizza,]);
    }
    ...
    public function pagar($id, $mes, $ano, Request $request) //atualiza um recurso informado mediante o parâmetro $id com os dados vindos do cliente, presentes no objeto Request;
    {
        ...
        $ano = $this->Enc->desencriptar($ano);
         ...
         return redirect()->route('mbcada.mes', ['mes' => $mes, 'ano' => $ano])->with('success', 'Realizado com sucesso!');
    }
     ...
     public function freeindex() // mostra a página inicial do recurso, normalmente uma listagem de recursos cadastrados;
     {
         ...
         ->get();
            ...
            return view('/mobi/painel', ['filtros' => $filtros, 'datae' => $datae, 'data3' => $data3,  'data1' => $data1, 'labelse' => $labelse, 'labels1' => $labels1, 'moviments' => $moviments, 'dinheiros' => $dinheiros, 'totalEntradaPorAno' => $totalEntradaPorAno, 'totalSaidaPorAno' => $totalSaidaPorAno, 'totalEntradaPorMes' => $totalEntradaPorMes, 'totalSaidaPorMes' => $totalSaidaPorMes, 'dadosPizza' => $dadosPizza, 'labels' => $labels, 'data2' => $data2]);
...
//=================================================================
              ...
              return view('/mobi/busca', ['filtros' => $filtros]);
                }
...
public function metastore(Request $request){
    ...
    return redirect()->route('mbcontrole.index', )->with('success', 'Meta cadastrada com sucesso!');
}
